...
```{r,}
library(CMEA)
Selectedprofiles <- mappingQueryTranscriptomic(Query_Transcriptomic_Profile, Transcriptomic_Profile, Cell_Morphology_Profile)
knitr::kable(head(Selectedprofiles[,1:3]))
```
...
```{r,}
library(CMEA)
cellMorphologyEnrichmentAnalysis <- cellMorphologyEnrichmentAnalysis(20)    
...
```{r,}
library(CMEA)
geneInteractionNetwrok <- geneInteractionNetwrok(10, 0.1, 0.6) 
knitr::kable(head(geneInteractionNetwrok))
```
